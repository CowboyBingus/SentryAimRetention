...
The build runs the aim, loader, registry and Windows memory-adapter checks, verifies the installable archive, audits the publication inventory, and creates `releases/SentryAimRetention.zip` and `releases/SentryAimRetention-source.zip`. In the shared mod workspace, both ZIPs go to the existing base `releases/` directory. Build output stays in ignored `build/`. Building does not install or launch the game.
...
```powershell
python -B scripts/privacy_audit.py --git --zip releases/SentryAimRetention.zip
```
